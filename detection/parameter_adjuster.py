...
class ParameterAdjuster:
    ...
    def adjust_canny_parameters(self, frame):
        def nothing(x):
            pass
    ...
    def adjust_trapezoid_parameters(self, frame):
        def nothing(x):
            pass
    ...
    def adjust_hough_parameters(self, processed_image, original_frame):
        def nothing(x):
            pass
        ...
        while True:
            ...
            lines = cv2.HoughLinesP(processed_image, 2, np.pi / 180, 50, minLineLength=self.lane_detector.min_line_length, maxLineGap=self.lane_detector.max_line_gap)
            averaged_lines = self.lane_detector.average_slope_intercept(original_frame, lines) if lines is not None else []
            line_image = self.lane_detector.display_lines(original_frame, averaged_lines)
    ...
    def adjust_all_parameters(self):
        ...
        self.adjust_hough_parameters(cropped_image, first_frame)
